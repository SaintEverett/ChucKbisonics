...
int port; // OSC port
int nSources; // how many things to compute
...
if( !me.args() ) 
{
    cherr <= "Input required, format is [nSources]:[address]:[port]:[hid]" <= IO.newline()
...
float myAmbi[nSources][9]; // multi dimensional array of coordinates
...
fun void hidVelocity() // spork used to set velocities of sound sources via arrow keys
{
    while( true )
    {
        hi => now;
